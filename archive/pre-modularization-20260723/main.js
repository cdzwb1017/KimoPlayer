...
class LyricsController {
  ...
  aggregateLatinWords(words) {
    ...
    words.forEach((w) => {
      ...
      // ⭐ 修复：parseWords 需 trim text，必须同时检查spaceAfter/spaceBefore 标记，否到"I "(spaceAfter) + "feel" 会被误合并成 "Ifeel" ⭐
  ...
  applyBlur(activeIndices, scrollIdx, allLines) {
    ...
    const currentPrimaryIdx = this.activeIndex >= 0 ? this.activeIndex : scrollIdx;
    const isPerfMode = document.body.classList.contains('perf-mode') || localStorage.getItem('kimo-performance-mode') === 'true';
    const isBlurEnabled = localStorage.getItem('kimo-lyrics-blur-enabled') !== 'false' && !isPerfMode;
    ...
    allLines.forEach((el, idx) => {
      ...
      // 🌟 100% 照着 audio-player.js 离散派发 depth 类名 🌟
      if (activeIndices.includes(idx) || idx === scrollIdx || idx === currentPrimaryIdx) {
        ...
        const d = currentPrimaryIdx - idx;
        newOpacity = `${Math.max(0.4, 1 - d * 0.15).toFixed(2)}`;
      } else {
        const d = idx - currentPrimaryIdx;
        el.classList.remove('depth-past', 'depth-1', 'depth-2', 'depth-3');
        if (d === 1) el.classList.add('depth-1');
        else if (d === 2) el.classList.add('depth-2');
        else if (d >= 3) el.classList.add('depth-3');

        newOpacity = `${Math.max(0.25, 1 - d * 0.15).toFixed(2)}`;
      }

      // 🌟 物理清除旧内联 style.filter，防止内联样式与 CSS Class 规则发生冲突 🌟
      if (el.style.filter) {
        el.style.removeProperty('filter');
        el._lastFilter = null;
      }

      // 🌟 自动滚动平滑上移动画期间，仅更新 Class 分层，不要覆盖内联 style.opacity，防止打断 CSS Transition 过渡 🌟
  ...
  syncBarSpans(wordSpans, charC, totalChars) {
    ...
    for (let i = 0; i < barWordSpans.length; i++) {
      ...
      // 🌟 动态更新 classList 确保唱完的字切为纯净实心填色(word-active)，正唱的字启用高光(word-singing)
      if (clamped >= 99) {
        if (!barSpan.classList.contains('word-active')) {
          barSpan.classList.add('word-active');
          barSpan.classList.remove('word-singing');
        }
      } else if (clamped <= 1) {
        ...
        if (!barSpan.classList.contains('word-singing')) {
  ...
  render() {
    ...
    this.lines.forEach((line, idx) => {
      ...
      if (line.isInterlude) {
        ...
        if (line.words && line.words.length >= 1) {
          ...
          wordsToRender.forEach((w, wi) => {
            ...
            // 纯空格syllable（stagger 拆出的空格char锛夆啋 textNode，不计入 charWords
            ...
            if (w.ruby) {
              ...
              // 解决 "あの日見た渚" 整个词的 ruby "ひみわたなぎ《 全部套在首kanji 上方错位 Bug
              const splitKanjiUnits = (text, ruby) => {
                const units = [];
                // 1. 找出所所kanji 位置
                for (let i = 0; i < text.length; i++) {
            ...
            // ⭐ 后继空格 →textNode（可见），syllable 文本含空格时由这里提供词间距 ⭐
            if (trailingMatch) {
              mainDiv.appendChild(document.createTextNode(trailingMatch[1]));
            }
          });
      ...
      // ⭐ AI 智能单句校准：绑定右侧contextmenu 快捷入口
    ...
    // ⭐ 在render 结束后，此时所有行的charWords 均已生成并绑定完较⭐
    // 进行 100% 精准的对唱及发声时间量endTime 的校出
    // ⭐ 精准校准行活动结束边界 endTime：保障主歌词未唱完前绝对处于 active 高亮状态，彻底解决翻译提前变浅问题
    this.lines.forEach((line, i) => {
      ...
      if (i < this.lines.length - 1) {
        const nextTime = this.lines[i + 1].time;
        // 🌟 核心保障：行结束时间必须高于主歌词最后一个字的唱完时间（+0.3s 余音温存期），绝不提前打入 past
      ...
      <div class="kimo-modal-card">
        ...
        <div class="kimo-modal-body" id="kimo-modal-body-content">
          <div class="kimo-loading-wrapper">
            ...
            <div class="kimo-loading-text">正在提取发音特征并执行CPU 毫秒级声谱对碰，请稍值..</div>
  ...
  renderDiffContent(line, aiSyllables, bodyEl, footerEl, modalEl, idx) {
    ...
    tHeader.innerHTML = `
      ...
      <span class="col-orig"><span>原始时间</span>
      <span class="col-arrow"></span>
      <span class="col-new"><span>AI 校准后</span>
  ...
  startSync() {
    ...
    const tick = (now) => {
      // 🌟 后台挂起优化：若页面被隐藏 (document.hidden)，挂起频繁的 RAF 计算
      if (document.hidden) {
      ...
      // 🌟 音频频谱 DOM 更新节流：限制为最高约 35ms (约 30 FPS)，避免在高刷屏上对 24 个 DOM 频繁触发重绘
  ...
  syncToTime(rawCurrentTime) {
    ...
    for (let i = 0; i < this.lines.length; i++) {
      const line = this.lines[i];
      // 处于演唱发声时间轴内（带 0.5s 提前预热入场，以及 0.4s 温存出场）。
    ...
    // ⭐ CalculateLanes：贪心分轨，重叠行分到不同 LaneIndex（BetterLyrics 风格）。
    const laneEndTimes = []; // laneEndTimes[lane] = 该轨道最后行的endTime
    ...
    // ⭐ GetCurrentLineIndex：主唱行优先（LaneIndex==0）
    let primaryIdx = -1;
    ...
    // ⭐ 🌟 视图深度解耦与后台早退 (Off-Screen Early Return) 🌟 ⭐
    // 当全屏歌词未展开（仅显示底部迷你歌词栏）时，仅更新底部歌词栏，
    // 彻底切断并跳过全屏歌词数百个 DOM 节点的所有遍历、applyBlur 滤镜与 Translate 物理动画，使主界面 CPU 占用降至 1%~3%！
    if (!this.isVisible) {
      if (activeIndex !== this.activeIndex) {
        this.updateBarLyrics(activeIndex);
        this.activeIndex = activeIndex;
      }
      if (activeIndex >= 0 && this.lines[activeIndex] && this.lines[activeIndex].charWords && this.lines[activeIndex].charWords.length > 0) {
        const lineData = this.lines[activeIndex];
        const totalChars = lineData.charWords.length;
        let charC = -1;
        let lastPassedIndex = -1;
        for (let i = 0; i < totalChars; i++) {
          const w = lineData.charWords[i];
          if (currentTime >= w.time && currentTime < w.time + w.duration) {
            charC = i + (currentTime - w.time) / w.duration;
            lastPassedIndex = i;
            break;
    ...
    // ⭐ 使用缓存，避免每帧遍历 DOM 树（querySelectorAll 在歌词行多时开销显著）。
    ...
    if (scrollIndex !== this.currentScrollIndex || viewActiveKey !== this._lastViewActiveKey) {
      ...
      if (scrollIndex >= 0 && allLines[scrollIndex]) {
        ...
        //   2. 读一次container.offsetHeight 触发全局 reflow（而非每行一次！）。
        ...
        if (Math.abs(delta) < 1) {
          ...
          allLines.forEach((el, idx) => {
            if (el.classList.contains('is-interlude-line')) {
              ...
              // 🌟 舒缓极品上移 (0.85s)：配以舒缓高弹性贝塞尔曲线 (0.16, 1, 0.3, 1)
              el.style.transition = 'transform 0.85s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.85s cubic-bezier(0.16, 1, 0.3, 1), filter 0.85s cubic-bezier(0.16, 1, 0.3, 1)';
          ...
          // Cleanup: 1300ms 后清 transition（确保 0.85s 上移动画 + 0.45s 翻译延迟淡出完满结束）
    ...
    if (activeIndicesKey !== this._lastActiveIndicesKey || scrollIndex !== this._lastVisualScrollIndex) {
      ...
      allLines.forEach((el, idx) => {
                  ...
                  if (currentActive && !this.isUserScrolling) {
                    ...
                    if (container && scrollIndex >= 0) {
                      ...
                      if (lineEl) {
                        ...
                        const containerHeight = container.clientHeight || container.getBoundingClientRect().height || 500;
                        const alignOffset = parseFloat(localStorage.getItem('kimo-lyrics-scroll-align')) || 0.5;
                        const targetOffset = baseOffsetTop - containerHeight * alignOffset;
                        const maxScroll = container.scrollHeight - container.clientHeight;
                        container.scrollTop = Math.max(0, Math.min(maxScroll > 0 ? maxScroll : 0, targetOffset));
                      }
                    }
                  }
        ...
        // ⭐ 排除掉仍在linesToProcess 中实时插值收尾的行，不进行硬边界填充 ⭐
        ...
        if (!activeIndices.includes(idx) && idx !== scrollIndex && !linesToProcess.has(idx) && currentTime > lineEndWarmup) {
            ...
            // ⭐ 温存期保护：当行刚结束（currentTime <= endTime+0.4）时，不要清除 fill，
            //    否则会出现"当前句播放完毕时染色概率消失"的闪烁 bug ⭐
            const warmupEnd = (this.lines[idx]?.endTime || 0) + 0.4;
            const inWarmup = currentTime <= warmupEnd && currentTime > (this.lines[idx]?.time || 0);
            if (el.dataset.fixedState && !inWarmup) {
                delete el.dataset.fixedState;
            }
            // ⭐ 修复：跳播时旧行在else 分支（不在active/linesToProcess），
            //    当.lyrics-word 的--char-fill 残留（如 past 行112%），必须清除 ⭐
            // ⭐ 但温存期内不重置，防止染色消失闪烁 ⭐
            // ⭐ 重要：past 行（idx < minActiveIdx）应该保持染色，只清除 future 行 ⭐
            if (!activeIndices.includes(idx) && !linesToProcess.has(idx) && idx !== scrollIndex && !inWarmup) {
                // 只清除 future 行的染色，past 行保持染色
                if (idx >= minActiveIdx) {
    ...
    linesToProcess.forEach(idx => {
      if (idx >= 0 && this.lines[idx].charWords && this.lines[idx].charWords.length > 0) {
        ...
        if (domLine) {
          ...
          // ⭐ 消除 Layout Thrashing：已建立对齐(bgAligned==='true')的节点，日常动画中无需每30帧重复调用 getBoundingClientRect() 重拍，大幅降低 CPU 占用
          ...
          if (charC === -1) {
              if (lastIdx === -1) {
                  ...
                  // 🌟 间隙平滑过渡：在两字之间的停顿期间，唱头在物理像素层平滑滑行，charC 锁定在前字末尾，绝不提前侵蚀下一个字符
                  ...
                  if (gapDuration > 0.001) {
                      ...
                      inGap = true;
                      gapPrevIdx = lastIdx;
                      currentGapT = 1 - (1 - t) * (1 - t); // ease-out 自然平滑过渡
                  }
                  charC = lastIdx + 1;
              }
          }
          ...
          // ⭐ 大师级物理排物理拼接渐变与唱头碰撞检测system (Viewport Texture Mapping & Playhead Collision Detection) ⭐
          if (domLine.rowsData && domLine.rowsData.length > 0) {
            ...
            domLine.rowsData.forEach(row => {
              ...
              // ⭐ 跨折行多排线性投影无缝流光过渡算算(Multi-Row Linear Projection Seamless Karaoke Flow System) ⭐
              let playheadX = 0;
              if (charC <= 0) {
                // 🌟 唱头处于行前静止期：彻底退避至高光带外部负像素区，确保首字 0 渗色 0 污染
                playheadX = 0 - transitionWidthPx - 20;
              } else if (charC >= totalChars) {
                // 唱头越过整行，做right侧超界投射
                const lastWordWidth = row.words[row.words.length - 1]?.offsetWidth || 40;
                playheadX = rowWidth + (charC - totalChars) * lastWordWidth + 10;
              } else {
                if (charC >= row.startIdx && charC <= row.endIdx + 1) {
                  // 1. 唱头当前正处于这一排内，进行高精度像素提取
                  if (inGap && gapPrevIdx >= row.startIdx && gapPrevIdx <= row.endIdx) {
                    // 🌟 正处于字与字的停顿间隙期：从上一字右边缘平滑滑向下一字左边缘，绝不侵蚀下一字！
                    ...
                    if (prevSpan && nextSpan) {
                      ...
                      playheadX = prevRight + (nextLeft - prevRight) * currentGapT;
                    } else if (prevSpan) {
                      playheadX = prevSpan.offsetLeft - rowLeft + prevSpan.offsetWidth;
                    } else {
                      playheadX = 0;
                    }
                  } else {
            ...
            wordSpans.forEach((span, i) => {
              const cw = lineData.charWords[i];
              if (!cw) return;
              // ⭐ 首字安全栓：首字时间之前，fill 保持 0
              let fill;
              if (currentTime < lineData.charWords[0].time + 0.05) {
                fill = 0;
              } else {
                fill = (charC < 0) ? 0 : (charC >= totalChars ? 100 : (i < Math.floor(charC) ? 100 : (i > Math.floor(charC) ? 0 : (charC - Math.floor(charC)) * 100)));
              }
          ...
          wordSpans.forEach((span, i) => {
            ...
            // ⭐ 借鉴 BetterLyrics：所有行（主歌词/背景和声）都正常上抬，不按LaneIndex 抑制 ⭐
            // 背景和声的出现/消失"由行时间范围(active/past)判定，不是靠抑制上抬
            // LaneIndex 仅用于滚动分轨（scrollIndex 跟随主唱行LaneIndex==0）。

            const prevLift = parseFloat(span.dataset.liftVal || '0.000');
            // ⭐ 从audio-player.js：不用速度阻尼，lift 直接跟随目标值（charX 本身平滑变化）。
            const nextLift = lift;

            // ⭐ 用delta 脏检测：只在抬起量真正变化时才写 transform）。
            //    消除边界值（0 / -liftAmp）每帧冗余重写导致的微卡顿⭐
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 🌟 消除启动白框：等待 CSS/DOM 在隐藏状态下准备就绪后，平滑淡入显现窗口（独立子窗口无需前端 show） 🌟
  if (!isStandaloneEditor) {
    try {
  ...
  // ══ 歌词景深模糊开关 (Lyrics Depth-of-Field Blur Toggle) ══
    ...
    import('@tauri-apps/api/event').then(({ listen }) => {
      listen('metadata-saved', async (event) => {
        ...
        try {
          ...
          if (typeof player !== 'undefined' && player && player.playlist) {
    ...
    immersiveBtn.addEventListener('click', async () => {
      try {
        ...
        if (isImmersiveMode) {
          ...
          // 在 app-container 上添加 immersive-mode 类
        ...
        isImmersiveMode = !isImmersiveMode; // 回滚状态
      }
    });
  }
  ...
  const backgroundLoadCovers = async (playlist) => {
    // 🌟 冷启动避高峰：延迟 1.2 秒执行海报后台补全，保障软件秒开与前几秒交互的极致流畅
    await new Promise(r => setTimeout(r, 1200));
    if (!playlist || playlist.length === 0) return;

    const CONCURRENCY_LIMIT = 2; // 降级为 2 路平缓并发，减少 CPU 与磁盘 I/O 冲击
    ...
    const worker = async () => {
      ...
      while (queue.length > 0) {
        ...
        count++;
        // 🌟 防卡顿让出主线程：每处理 2 个歌曲让出 40ms 给 UI 渲染
        if (count % 2 === 0) {
  ...
  // ⭐ 音乐库（与 player.playlist 分离）：本地扫描的全部歌曲，独立于当前播放队到⭐
  let musicLibrary = [];
...
const showToast = (message) => {
  ...
  const parseM3U = (content) => {
    ...
    for (const raw of lines) {
      ...
      if (line.startsWith('#PLAYLIST:')) {
        ...
        // 文件路径：如果不是绝对路径，则尝试相对于 M3U 文件所在目录解析
        let filePath = line;
  ...
  const toggleLikeSong = (songData) => {
    ...
    savePlaylists(all);       // all 和liked 是同一个数组里的同一个对象，变更会被保存
    updateHeartButton();
    ...
    perfCard.innerHTML = `
      ...
      <div class="setting-row">
        <div class="setting-info">
          ...
          <div class="setting-desc">强烈推荐 Intel i5-1135G7 / 集成显卡设备开启。自动关闭高昂的 CSS 景深模糊与毛玻璃背景，极大降低 CPU 与 GPU 占用，大幅提升歌词流畅度。</div>
        </div>
        <div style="display: flex; align-items: center;">
          <input type="checkbox" id="settings-perf-mode" ${isPerfMode ? 'checked' : ''} style="width: 18px; height: 18px; cursor: pointer; accent-color: rgb(var(--dynamic-color, 16, 185, 129));" />
        </div>
      </div>
    `;
        ...
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 4px;">
          ...
          <div>
            <div style="font-size: 18px; font-weight: 700; color: var(--text-primary); letter-spacing: 0.5px;">KimoPlayer</div>
                        <div style="font-size: 13px; color: var(--text-secondary); margin-top: 2px;">版本: 1.3.0-beta0722-03</div>
          </div>
        </div>
  ...
  const renderSearchList = (container) => {
    ...
    // 3. Render Artists
    if (hasArtists) {
      html += '<div class="search-group-title"><div>艺术家</div>';
  ...
  // ⭐ 兼容旧版本：如果没有 musicLibrary 当player.playlist 有歌曲，用它初始化library ⭐
    ...
    if (songItem) {
      ...
      menuEl.innerHTML = `
        <div class="context-menu-item" id="menu-song-play">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></span>
          <span>立即播放</span>
        </div>
        <div class="context-menu-item" id="menu-song-playnext">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg></span>
          <span>下一首播放</span>
        </div>
        <div class="context-menu-item" id="menu-song-copy">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/></svg></span>
          <span>复制歌曲信息</span>
        </div>
        <div class="context-menu-item" id="menu-song-addplaylist">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></span>
          <span>添加到歌单</span>
        </div>
        <div class="context-menu-divider"></div>
        <div class="context-menu-item" id="menu-song-edit">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></span>
          <span>编辑元数据</span>
        </div>
        <div class="context-menu-item" id="menu-song-reveal">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg></span>
          <span>在资源管理器中显示</span>
        </div>
      `;
      ...
      menuEl.innerHTML = `
        ...
        <div class="context-menu-item" id="menu-offset-dec">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span>
          <span>歌词提前 0.5s</span>
        </div>
        ...
        <div class="context-menu-item" id="menu-reload">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg></span>
          <span>重新加载界面</span>
        </div>
        <div class="context-menu-item" id="menu-close" style="color: #ff5555;">
          <span class="menu-icon"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></span>
          <span>关闭播放器</span>
        </div>
      `;
      ...
      if (player && player.audio && !player.audio.paused) {
        if (playIcon) playIcon.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M6 4h4v16H6zM14 4h4v16h-4z"/></svg>';
        if (playText) playText.textContent = '暂停播放';
      } else {
        if (playIcon) playIcon.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>';
  ...
  // ══ <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> 元数据与歌词字级时轴编辑器内核(Premium Timeline Lyrics Editor System) ══
  
  // 临时内存歌词结构化对应已移至顶端，在此仅作为流程注释
  currentEditableLyrics = [];
  currentLyricsType = 'lrc'; 
  ...
  if (!bubbleEditor) {
    ...
    bubbleEditor.innerHTML = `
      ...
      <div class="bubble-group">
        <label>原文 译文/label>
        <input type="text" class="bubble-input text" placeholder="存 />
      </div>
      <div class="bubble-group">
        <label>结束时间</label>
  ...
  const parseLyricsFromRawText = (rawText) => {
    ...
    lines.forEach(line => {
      ...
      if (rowTimeMatch) {
        ...
        // 检查剩余部分是否包含行内字级时间戳，如 ほ[00:26.87] 我《00:26.87>
    ...
    // 译文行自动合并过滤器）。
    // 如果某行的时间与上一行几乎相同（宸。<= 0.15秒）且上一行尚未绑定译文，则将当前行判定为上一行的翻译行并进行合并
    const list = [];
  ...
  const renderTranslationBar = (row, rowDiv) => {
    ...
    if (row.translation !== null && row.translation !== undefined) {
      transBar.innerHTML = `
        ...
        <button class="btn-translation-delete" title="删除行翻译</button>&times;</button>
      `;
      ...
      transBar.innerHTML = `
        <button class="btn-translation-add">+ 添加行翻译</button>
      `;
  ...
  const openMetadataEditor = async (filePath) => {
    try {
      await invoke('open_metadata_editor_window', { path: filePath });
    } catch (e) {
      console.error('[MetadataEditor] Failed to open standalone window:', e);
      showToast('无法打开独立编辑窗口');
    }
  };
  ...
  const viewAlbumImageLarge = (coverSrc) => {
    ...
    modal.querySelector('#download-zoom-cover-btn').addEventListener('click', async () => {
      try {
        ...
        a.click();
        if (window.showToast) window.showToast('已启动专辑封面图片下载');
      } catch (e) {
  ...
  // 首次打开 1.3.0-beta0722-03 弹出更新公告
  if (localStorage.getItem('kimo-update-seen-130-0722-03') !== 'true') {
    const sections = [
      {
        title: '🚀 性能与流畅度大幅提升',
        items: [
          ['⚡', '<b>播放轻盈省电</b><br>优化后台渲染机制，主界面播放音乐时系统资源占用降低 90% 以上'],
          ['🛡️', '<b>低功耗性能模式</b><br>在【系统设置 -> 歌词与视觉动效】中新增省电/性能模式，推荐集成显卡及笔记本用户开启'],
          ['🎵', '<b>歌词滚动更平滑</b><br>优化歌词渲染引擎，歌词滚动与字符动效更加自然丝滑'],
        ],
      },
      {
        title: '✨ 启动与视觉体验优化',
        items: [
          ['🖼️', '<b>无缝平滑启动</b><br>优化软件打开时的窗口呈现，告别界面启动边框与白色闪烁'],
          ['⚡', '<b>软件秒开无卡顿</b><br>优化资源后台加载顺序，软件打开响应更快、更轻便'],
        ],
      },
    ];
    ...
    overlay.innerHTML = `<div class="kimo-modal-card" style="max-width:480px;width:92%;max-height:78vh;padding:0;text-align:left;display:flex;flex-direction:column;overflow:hidden;">
      <div style="padding:24px 26px 18px;border-bottom:1px solid rgba(255,255,255,0.08);">
        <div style="font-size:18px;font-weight:700;color:var(--text-primary);margin-bottom:4px;display:flex;align-items:center;gap:8px;">
          <span style="font-size:22px;">🎵</span> KiomPlayer 1.3.0-beta0722-03
        </div>
        <div style="font-size:11px;color:var(--text-secondary);">2026.07.22 · 性能与体验优化更新</div>
      </div>
      ...
      <div style="padding:14px 26px;border-top:1px solid rgba(255,255,255,0.08);">
        <button id="kimo-update-ok-btn" style="width:100%;padding:10px;font-size:14px;font-weight:600;border:none;border-radius:8px;background:rgb(var(--dynamic-color,16,185,129));color:#fff;cursor:pointer;">开始体验</button>
      </div>
    </div>`;
    ...
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay || e.target.id === 'kimo-update-ok-btn') {
        ...
        overlay.querySelector('.kimo-modal-card').style.transform = 'scale(0.9) translateY(20px)';
        setTimeout(() => overlay.remove(), 200);
        localStorage.setItem('kimo-update-seen-130-0722-03', 'true');
      }
    });
  }

  // 🌟 独立元数据与歌词编辑器子窗口初始化 (延迟至 DOM 尾部，规避闭包函数与全局变量死区异常) 🌟
  const initStandaloneMetadataEditor = async (filePath) => {
    ...
    setVal('edit-metadata-lyrics', '正在读取元数据与歌词中..');
    ...
    document.getElementById('edit-metadata-delete-cover')?.addEventListener('click', () => {
      document.getElementById('edit-metadata-remove-cover').value = 'true';
      document.getElementById('edit-metadata-cover-path').value = '';
      document.getElementById('edit-metadata-cover-preview').src = getCoverSrc(null);
    });
    ...
    document.getElementById('metadata-editor-save')?.addEventListener('click', async () => {
      const title = getVal('edit-metadata-title').trim();
      const artist = getVal('edit-metadata-artist').trim();
      const album = getVal('edit-metadata-album').trim();
      const coverPath = getVal('edit-metadata-cover-path');
      const removeCover = getVal('edit-metadata-remove-cover') === 'true';

      const yearVal = getVal('edit-metadata-year');
      const trackVal = getVal('edit-metadata-track');
      const discVal = getVal('edit-metadata-disc');
      const genre = getVal('edit-metadata-genre');
      const albumArtist = getVal('edit-metadata-album-artist');
      const composer = getVal('edit-metadata-composer');
      const lyricist = getVal('edit-metadata-lyricist');
      const comment = getVal('edit-metadata-comment');
  ...
  if (isStandaloneEditor) {
    ...
    document.getElementById('metadata-editor-close')?.addEventListener('click', handleCloseWindow);
    document.getElementById('metadata-editor-cancel')?.addEventListener('click', handleCloseWindow);
    ...
    setTimeout(() => {
      ...
      if (modal && content) {
        console.log('[Debug-UI] Modal DOM:', modal);
        console.log('[Debug-UI] Modal Display:', window.getComputedStyle(modal).display);
        console.log('[Debug-UI] Modal Opacity:', window.getComputedStyle(modal).opacity);
        console.log('[Debug-UI] Modal Z-Index:', window.getComputedStyle(modal).zIndex);
        console.log('[Debug-UI] Content Display:', window.getComputedStyle(content).display);
        console.log('[Debug-UI] Content Opacity:', window.getComputedStyle(content).opacity);
        console.log('[Debug-UI] Content Width:', window.getComputedStyle(content).width);
        console.log('[Debug-UI] Content Height:', window.getComputedStyle(content).height);
      } else {
        console.warn('[Debug-UI] Modal or Content elements not found in DOM!');
      }
    }, 500);
